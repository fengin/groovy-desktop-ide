...
let currentDocBizCode = ''; // 当前文档抽屉展示 of bizCode
...
async function runScript() {
  ...
  try {
    ...
    if (contentType.includes('application/octet-stream')) {
      ...
      if (window.__TAURI_INTERNALS__) {
        // Desktop: 用 Tauri 文件 API 写入系统"下载"文件夹
        try {
          const { writeFile, BaseDirectory } = await import('@tauri-apps/plugin-fs');
          await writeFile(filename, new Uint8Array(arrayBuffer), { baseDir: BaseDirectory.Download });
          appendOutput(`✅ 文件已保存到下载文件夹: ${filename} (${arrayBuffer.byteLength} bytes)`, 'success');
          showToast('文件已保存到下载文件夹: ' + filename, 'success');
        } catch (fsErr) {
          // plugin-fs 不可用时，降级为 blob 下载
          console.warn('plugin-fs 写入失败，降级为 blob 下载:', fsErr);
          triggerBlobDownload(arrayBuffer, filename);
          appendOutput(`✅ 文件已下载: ${filename} (${arrayBuffer.byteLength} bytes)`, 'success');
        }
      } else {
        // 开发模式（非 Tauri 环境）: blob 下载
        triggerBlobDownload(arrayBuffer, filename);
        appendOutput(`✅ 文件已下载: ${filename} (${arrayBuffer.byteLength} bytes)`, 'success');
      }
...
/** 通过 blob URL + <a> 标签触发浏览器下载 */
function triggerBlobDownload(arrayBuffer, filename) {
